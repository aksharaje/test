...
from app.services.qualtrics_service import qualtrics_service
...
def start_oauth(
    ...
    url = qualtrics_service.get_oauth_url(return_url)
    return {"url": url}
...
async def oauth_callback(
    ...
    try:
        result = await qualtrics_service.handle_oauth_callback(db, code, state)
        ...
        print(f"Qualtrics OAuth Error: {e}")
        return RedirectResponse(f"http://localhost:4200/settings/integrations?error=Failed to connect Qualtrics")
