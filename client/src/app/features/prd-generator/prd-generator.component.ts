      ...
      <div class="w-1/2 flex flex-col bg-muted/30">
        ...
        <div class="border-b bg-background p-4">
          ...
          <p class="mt-1 text-sm text-muted-foreground">
             View and manage your past PRDs
          </p>
        </div>
        
        <!-- History List -->
        <div class="flex-1 overflow-y-auto">
          @if (service.prds().length === 0) {
             <div class="flex flex-col items-center justify-center py-8 text-center px-4 h-full">
                <ng-icon name="lucideHistory" class="mb-4 h-12 w-12 text-muted-foreground/50" />
                <h3 class="text-lg font-medium text-muted-foreground">No history yet</h3>
                     ...
                     <div class="flex-1 min-w-0">
                       <div class="flex items-center gap-2 mb-1">
                         <span
                           ...
                           [class.bg-green-100]="prd.status === 'final'"
                           [class.text-green-700]="prd.status === 'final'"
                           [class.bg-orange-100]="prd.status === 'draft'"
                           [class.text-orange-700]="prd.status === 'draft'"
                           [class.bg-blue-100]="prd.status === 'processing'"
                           [class.text-blue-700]="prd.status === 'processing'"
                           [class.bg-red-100]="prd.status === 'failed'"
                           [class.text-red-700]="prd.status === 'failed'"
                           [class.bg-gray-100]="prd.status === 'pending'"
                           [class.text-gray-700]="prd.status === 'pending'"
                         >
                           <ng-icon 
                            [name]="service.statusMap[prd.status].icon" 
                            [class]="service.statusMap[prd.status].class + ' h-3 w-3 mr-1'"
                           />
                           {{ service.statusMap[prd.status].label }}
                         </span>
                         <span class="text-xs text-muted-foreground">
                           {{ prd.createdAt | date:'short' }}
                         </span>
                       </div>
                       <h3 class="font-medium text-foreground line-clamp-2 text-sm">{{ prd.title || 'Untitled PRD' }}</h3>
                     </div>
                     <div class="flex items-center gap-1 ml-2">
                       @if (prd.status === 'failed') {
                         <button 
                           (click)="retryPrd(prd.id, $event)"
...
export class PrdGeneratorComponent implements OnInit {
  ...
  async generatePrd(): Promise<void> {
    ...
    if (prd) {
      // Navigate to output page immediately
  ...
  // History actions
  async retryPrd(id: number, event: Event): Promise<void> {
    event.stopPropagation();
    const prd = await this.service.retryPrd(id);
    if (prd) {
      this.router.navigate(['/prd-generator/output', prd.id]);
    }
  }

  async deletePrd(id: number, event: Event): Promise<void> {
    event.stopPropagation();
    if (confirm('Are you sure you want to delete this PRD?')) {
      await this.service.deletePrd(id);
    }
  }

  viewPrd(id: number): void {
    this.router.navigate(['/prd-generator/output', id]);
  }
}
